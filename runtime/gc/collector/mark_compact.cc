...
bool MarkCompact::DoPageCompactionWithStateChange(size_t page_idx,
  ...
  if (kMode == kFallbackMode || moving_pages_status_[page_idx].compare_exchange_strong(
    ...
    if (kMode == kCopyMode) {
      if (map_immediately) {
        CopyIoctl(to_space_page, page, kPageSize);
        ...
        uint32_t store_val = (page - from_space_begin_) / kPageSize;
...
bool MarkCompact::FreeFromSpacePages(size_t cur_page_idx, int mode, size_t end_idx_for_mapping) {
  ...
  if (size > kMinFromSpaceMadviseSize) {
    ...
    if (cur_page_idx < end_idx_for_mapping) {
      ...
      for (size_t i = cur_page_idx + len / kPageSize; i < end_idx_for_mapping; i++) {
        PageState state = static_cast<PageState>(
    ...
    const ssize_t gBufferPages = 4 * kPageSize;
  ...
  while (idx > 0) {
    ...
    if (kMode == kMinorFaultMode) {
      ...
      if (cur_reclaimable_page_ > last_reclaimable_page_) {
        cur_reclaimable_page_ -= kPageSize;
  ...
  while (true) {
    ...
    if (fault_addr == conc_compaction_termination_page_) {
      ...
      if (!gKernelHasFaultRetry || ret == 1) {
        ZeropageIoctl(fault_addr, kPageSize, /*tolerate_eexist=*/false, /*tolerate_enoent=*/false);
      } else {
...
void MarkCompact::ConcurrentlyProcessMovingPage(uint8_t* fault_page,
  ...
  if (kMode == kCopyMode && fault_page >= unused_space_begin) {
    ...
    ZeropageIoctl(fault_page, kPageSize, /*tolerate_eexist=*/true, /*tolerate_enoent=*/true);
    return;
  }
  ...
  if (first_obj == nullptr) {
    ...
    uint8_t* end = AlignDown(thread->GetTlabEnd(), kPageSize);
    if (fault_page < thread->GetTlabPos() || fault_page >= end) {
      end = fault_page + kPageSize;
    }
    size_t end_idx = page_idx + (end - fault_page) / kPageSize;
    size_t length = 0;
    for (size_t idx = page_idx; idx < end_idx; idx++, length += kPageSize) {
    ...
    if (length > 0) {
      ...
      for (size_t len = 0, idx = page_idx; len < length; idx++, len += kPageSize) {
  ...
  while (true) {
    ...
    if (state == PageState::kProcessing || state == PageState::kMutatorProcessing ||
      ...
      if (state == PageState::kUnprocessed &&
        ...
        if (kMode == kCopyMode) {
          CopyIoctl(fault_page, buf, kPageSize);
      ...
      if (state == PageState::kProcessed) {
        ...
        if (MapMovingSpacePages(page_idx, arr_len) >= kPageSize) {
          break;
        }
        raw_state = moving_pages_status_[page_idx].load(std::memory_order_acquire);
      }
  ...
  while (start_page < end_page) {
    ...
    if (map_len == 0) {
      ...
      while (start_state->load(std::memory_order_relaxed) != PageState::kProcessed && length > 0) {
        start_state++;
        length -= kPageSize;
        start_shadow_page += kPageSize;
        start_page += kPageSize;
      }
    } else {
      ...
      if (use_uffd_sigbus_) {
        ...
        for (size_t l = 0; l < map_len; l += kPageSize, start_state++) {
...
void MarkCompact::ConcurrentlyProcessLinearAllocPage(uint8_t* fault_page, bool is_minor_fault) {
  ...
  if (arena_iter == linear_alloc_arenas_.end() ||
    ...
    ZeropageIoctl(fault_page, kPageSize, /*tolerate_eexist=*/true, /*tolerate_enoent=*/false);
  } else {
            ...
            if (kMode == kCopyMode || is_minor_fault) {
              ...
              if (kMode == kCopyMode) {
                if (updater.WasLastPageTouched()) {
                  ...
                  // in the kernel.
                  ZeropageIoctl(
                      fault_page, kPageSize, /*tolerate_eexist=*/false, /*tolerate_enoent=*/false);
...
void MarkCompact::ProcessLinearAlloc() {
  ...
  auto map_pages =
      [this, &diff, &space_data, &unmapped_range_start, &unmapped_range_end, &state_arr]() {
        ...
        DCHECK_ALIGNED_PARAM(unmapped_range_end - unmapped_range_start, kPageSize);
        size_t page_idx = (unmapped_range_start - space_data->begin_) / kPageSize;
        ...
        if (state_arr[page_idx].compare_exchange_strong(
          ...
          if (!minor_fault_initialized_) {
            ...
            if (updater.WasLastPageTouched()) {
              ...
              // in the kernel.
              ZeropageIoctl(
                  page_begin, kPageSize, /*tolerate_eexist=*/false, /*tolerate_enoent=*/false);
...
void MarkCompact::CompactionPhase() {
  ...
  if (CanCompactMovingSpaceWithMinorFault()) {
    ...
    if (used_size < moving_space_size) {
      ...
      ZeropageIoctl(
          unused_first_page, kPageSize, /*tolerate_eexist*/ true, /*tolerate_enoent*/ false);
