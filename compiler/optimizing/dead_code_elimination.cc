...
struct HDeadCodeElimination::TryBelongingInformation : public DeletableArenaObject<kArenaAllocDCE> {
  ...
  for (auto it = tries.begin(); it != tries.end(); it++) {
    ...
    for (auto other_it = next(it); other_it != tries.end(); /*other_it++ in the loop*/) {
      ...
      if (try_boundary->HasSameExceptionHandlersAs(*other_try_boundary)) {
        ...
        // there's no need to call `delete` since TryBelongingInformation is a DeletableArenaObject.
